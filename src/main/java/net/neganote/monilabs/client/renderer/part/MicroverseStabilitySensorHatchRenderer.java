...
public class MicroverseStabilitySensorHatchRenderer extends MachineRenderer {

    private final String texturePath;

    public MicroverseStabilitySensorHatchRenderer(String texturePath) {
        super(MoniLabs.kjsResLoc("block/microverse_casing"));
    ...
    public void renderMachine(List<BakedQuad> quads, MachineDefinition definition, @Nullable MetaMachine machine,
        ...
        if (side == frontFacing && modelFacing != null) {
            ...
            int fillLevel = 0;
            if (machine instanceof MicroverseStabilitySensorHatchPartMachine integritySensor) {
                var controllers = integritySensor.getControllers().stream()
                        .filter(MicroverseProjectorMachine.class::isInstance)
                        .map(MicroverseProjectorMachine.class::cast)
                        .toList();
                if (!controllers.isEmpty()) {
                    var controller = controllers.get(0);
                    if (controller.isFormed()) {
                        fillLevel = (int) (4 * controller.getMicroverseIntegrity() /
